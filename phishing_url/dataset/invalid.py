...
# CSV 파일로 저장하는 함수
def save_to_csv(data, filename='phishing_links.csv'):
    with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['Index', 'Extracted Link']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

        writer.writeheader()
        for index, link in enumerate(data, start=1):  # 인덱스 추가
            writer.writerow({'Index': index, 'Extracted Link': link})
